...
- (void)viewWillAppear:(BOOL)animated {
	...
	NSLog(@"Delegate: %@", appDelegate.tableBuddyList);
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
	MSGSAppDelegate* appDelegate = (MSGSAppDelegate *)[[UIApplication sharedApplication] delegate];\
	if ([appDelegate tableBuddyList] == nil) {
		return 0;
	} else {
		return [[appDelegate tableBuddyList] count];
	}
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // Set up the cell...
	MSGSAppDelegate *appDelegate = (MSGSAppDelegate *)[[UIApplication sharedApplication] delegate];
	[cell setText:[[appDelegate tableBuddyList] objectAtIndex:indexPath.row]];
